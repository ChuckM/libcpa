/*
 * Copyright (c) 2013 - 2014 Chuck McManis, all rights reserved.
 *
 ...
 */
static uint32_t uart_pin_map(enum GPIO_PORT_PIN pin, enum PIN_ATTRIBUTE attr);
 ...
 *
 * The function uart_pin_map takes a GPIO_PORT_PIN and can return
 ...
 */
static uint32_t
pin_uart_attr(enum GPIO_PORT_PIN pin, int attr) {
    switch (pin) {
        case PA0:
        case PA1:
            switch (attr) {
        ...
        default: return 0;  // Isn't a UART or USART pin
    }
}
 ...
 */
static uint32_t
pin_to_af(enum GPIO_PORT_PIN pin) {
    switch (pin) {
        case PA0:
        case PA1:
        case PC6:
 ...
 */
static uint32_t
uart_pin_map(enum GPIO_PORT_PIN pin, enum PIN_ATTRIBUTE attr) {
    switch (attr) {
        case USART:
